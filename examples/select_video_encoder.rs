...
use std::{env, io};
use windows_record::{
    enumerate_video_encoders, get_preferred_video_encoder_by_type, Recorder, RecorderConfig,
    RecorderError, Result, VideoEncoderType,
};
    ...
    let selected_encoder = if encoder_idx > 0 && encoder_idx <= encoders.len() {
        ...
        get_preferred_video_encoder_by_type(VideoEncoderType::H264)
            ...
            .ok_or_else(|| {
                ...
                RecorderError::Generic("No suitable video encoders found on the system".to_string())
            })?
    };

    info!(
        "Selected encoder: {} ({:?})",
        selected_encoder.name, selected_encoder.encoder_type
    );
