...
pub unsafe fn create_sink_writer(
    ...
    // Create sink writer
    info!("create_sink_writer - Creating sink writer from URL: {}", output_path);
...
unsafe fn configure_mixed_audio_stream(
    ...
    info!("configure_mixed_audio_stream - Starting for stream index {}", stream_index);
    ...
    info!("configure_mixed_audio_stream - Setting input media type for stream {}", stream_index);
...
unsafe fn configure_video_stream(
    ...
    info!("configure_video_stream - Creating encoder configuration with bitrate {}", video_bitrate);
...
unsafe fn create_video_output_type(
    ...
    info!("create_video_output_type - Starting with fps={}/{}, resolution={}x{}, encoder={:?}", 
    ...
    info!("create_video_output_type - Setting MF_MT_FRAME_RATE to {}/{}", fps_num, fps_den);
    output_type.SetUINT64(
        &MF_MT_FRAME_RATE,
        ((fps_num as u64) << 32) | (fps_den as u64),
    )?;
    
    info!("create_video_output_type - Setting MF_MT_FRAME_SIZE to {}x{}", output_width, output_height);
...
unsafe fn create_video_input_type(
    ...
    info!("create_video_input_type - Starting with fps={}/{}, resolution={}x{}", 
    ...
    info!("create_video_input_type - Setting MF_MT_FRAME_RATE to {}/{}", fps_num, fps_den);
    input_type.SetUINT64(
        &MF_MT_FRAME_RATE,
        ((fps_num as u64) << 32) | (fps_den as u64),
    )?;
    
    info!("create_video_input_type - Setting MF_MT_FRAME_SIZE to {}x{}", output_width, output_height);
    ...
    info!("create_video_input_type - Setting MF_MT_DEFAULT_STRIDE to {}", output_width);
...
unsafe fn create_encoder_config(video_bitrate: u32) -> Result<Option<IMFAttributes>> {
    info!("create_encoder_config - Starting with bitrate {}", video_bitrate);
    ...
    if let Some(attrs) = &config_attrs {
        ...
        info!("create_encoder_config - Setting CODECAPI_AVEncCommonMeanBitRate to {}", video_bitrate);
